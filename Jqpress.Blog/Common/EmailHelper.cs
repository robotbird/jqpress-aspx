...
namespace Jqpress.Blog.Common
{
    ...
    public class EmailHelper
    {
        ...
        public static void Send(string recipients, string subject, string body)
        {
            try
            {
                MailAddress from = new MailAddress(BlogConfig.GetSetting().SmtpEmail, BlogConfig.GetSetting().SiteName);
                ...
                smtp.Host = BlogConfig.GetSetting().SmtpServer;
                smtp.Credentials = new System.Net.NetworkCredential(BlogConfig.GetSetting().SmtpUserName, BlogConfig.GetSetting().SmtpPassword);
                smtp.EnableSsl = Convert.ToBoolean(BlogConfig.GetSetting().SmtpEnableSsl);
                smtp.Port = BlogConfig.GetSetting().SmtpServerPost;
                smtp.Send(m);
            }
            catch
            { }
